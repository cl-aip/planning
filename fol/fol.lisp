...
(cl:defpackage :fol  ; by Seiji
  ...
  (:export compound-unify compound-unify? +no-bindings+)
  )

(in-package :fol) ; by Seiji

(eval-when (:execute :load-toplevel :compile-toplevel) ; by Seiji
(defstruct fol-kb
...
(defmethod tell ((kb fol-kb) sentence)
  ...
  (loop for clause in (->minimal-cnf sentence) do
...
;;;  "Use resolution to decide if sentence <query> is true."
...
;;;      (loop for literal in clause do
;;;           (loop for r in (possible-resolvers kb literal) do
;;;                (let ((b (unify ??? literal)))
;;;                  (when b
