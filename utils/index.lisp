...
(defun gen-index (&optional (system-names *planning-system-names*))
  ...
  (gen-entries (target-file "entries" :planning :type "html" :path '("doc"))))
  ...
  (dolist (name system-names)
    (unless (member name '(all everything))
      (with-open-file (stream (target-file 
                               (format nil "overview-~A" name) :planning
                               :type "html" :path '("doc"))
                              :direction :output :if-exists :supersede)
        (format t "~&Working on system ~A~%" (setq *planning-system* name))
        (do-file (line (target-file "README" :planning :type "html"
...
(defun gen-files-overview (name stream)
  ...
  ;; List all the files:
  (operate-on-target-system :planning
   name
   #'(lambda (file)
       (format stream "<LI><A HREF=\"#~A\"><B>~A.cl</B></A> ~A"
         (target-namestring file :planning)
         (pathname-name file) (or (file-title file) "")))
   :directory-operation
   #'(lambda (path)
       (format stream "</UL><A HREF=\"../~A\"><B>~:*~A</B></A>:~%<UL> "
         (target-namestring (target-file nil :planning :type nil :path path) :planning))))
  (format stream "</UL>~%~%")
  ...
  (operate-on-target-system :planning
   ...
   #'(lambda (file)
       ...
       (overview-entries file :planning stream)))
...
(defun gen-entries (entries.html)
  ...
  (let* ((entries (alphabetize *user-symbols* 
    ...
    (with-open-file (stream entries.html :direction :output 
                            :if-exists :supersede)
      (format stream "<HTML><TITLE>Index of Planning Code</TITLE>
 <BODY bgcolor=\"#ffffff\">
 <H1>Index of Planning Code</H1>~%")
  ...
  <A HREF=\"https://franz.com/support/documentation/10.1/ansicl/ansicl.htm\">
  ANSI Common Lisp Documentation</A> (based on the ANSIhttps://franz.com/support/documentation/10.1/ansicl/ansicl.htm standard document).
  ...
  <A HREF=\"#Y\"> Y</A> <A HREF=\"#Z\"> Z</A> </CENTER>~%<PRE>~%" 
        (length symbols))
      (format stream "Planning Symbols~VTCommon Lisp Symbols~%" width)
...
(defun print-structure (exp stream depth)
  ...
  (cond ((equal (semi-comment-string exp)                        ; by Seiji
                " ----------------------------------------------------------------------------------")
...
(defun overview-entries (file system stream)
  ...
  (format stream "<A NAME=\"~A\"><HR>~%<H2>File <A HREF=\"../~A\">~A</A></H2></A>~%"  
    (target-namestring file system) (target-namestring file system) (target-namestring file system))
...
(defun overview-exp (exp stream file)
  ...
  (cond
   ((and (semi-comment-p exp) (>= (semi-comment-level exp) 3)
    ...
    (print-structure exp stream nil))
   ((consp exp)
  ...
  (let ((apos1 (position #\< str)))
    (cond (apos1 (multiple-value-bind (word newpos) (angle-bracket-close str (1+ apos1))
                   (ecase (cond ((cl:member word args :test #'string=) :italic)
                                ((or (fboundp (intern word :callcc))
                                     (fboundp (intern word :utils))
                                     (fboundp (intern word :fol))
                                     (fboundp (intern word :plan)))
                                 :bold)
                                ((or (boundp (intern word :callcc))
                                     (boundp (intern word :utils))
                                     (boundp (intern word :fol))
                                     (boundp (intern word :plan)))
                                 :bold)
                                (t :italic))
  ...
  (let ((apos1 (position #\< str))
    ...
    (cond ((and apos1 (> apos1 0) (or (null apos2) (< apos1 apos2)) (char= #\\ (char str (1- apos1))))
          ...
          (apos1 (multiple-value-bind (word newpos) (angle-bracket-close str (1+ apos1))
                   (ecase (cond ((or (fboundp (intern word :callcc))
                                     (fboundp (intern word :utils))
                                     (fboundp (intern word :fol))
                                     (fboundp (intern word :plan)))
                                 :bold)
  ...
  (let ((entry (get-entry name *user-symbols*)))
    ...
    (if (numberp file-or-page)
      ...
      (push (make-def :kind kind :arg-type arg-type :system *planning-system*
                      :file (target-namestring file-or-page :planning))
            (entry-defs entry)))))
